...
export class DatabaseService {
  ...
  public static async getUserInfoFromDatabase(userId: number): Promise<{ [key: string]: string }> {
    if (!this.dbInstance) {
  ...
  public static async getPostsTitlesByAuthor(authorId: number): Promise<string[]> {
    if (!this.dbInstance) {
      this.dbInstance = await open({
        filename: '../../database.sqlite',
        driver: sqlite3.Database,
